export default class FilltextService {
  ...
  async getLiteData(num) {
    ...
    return res.map(this._transformList)[num];
  };

  async getFullData() {
    const res = await this.getResource('?rows=1000&id=%7Bnumber%7C1000%7D&firstName=%7BfirstName%7D&delay=3&lastName=%7BlastName%7D&email=%7Bemail%7D&phone=%7Bphone%7C(xxx)xxx-xx-xx%7D&address=%7BaddressObject%7D&description=%7Blorem%7C32%7D');
    return res;
  };
